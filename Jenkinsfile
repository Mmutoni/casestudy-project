    ...
    stages{
        ...
        stage('upload artifact to jfrog'){
            steps{
                ...
                ansible-dev/ansible-${BUILD_ID}.zip "http://54.147.142.41:8081/artifactory/ansible-playbook/ansible-${BUILD_ID}.zip"'
            }
        }
        stage('publish to ansible server'){
            steps{
                sshPublisher(publishers:\
                    ...
                    remoteDirectory: '.', \
                    remoteDirectorySDF: false, removePrefix: '', sourceFiles: 'ansible-dev/ansible-${BUILD_ID}.zip')], \
